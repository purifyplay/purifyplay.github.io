...
async function main() {
  if (window.location.hash) {
    ...
    if ("h" in params) {
      hint = params["h"];
      password = prompt(`Por favor ingrese la contraseña para desbloquear el enlace.\n\nHint: ${hint}`);
    } else {
      password = prompt("Por favor ingrese la contraseña para desbloquear el enlace.");
    }
    ...
    try {
      ...
      document.querySelector("#no-redirect").href =
        `https://purifyplay.github.io/redirection/site/decrypt/#${hash}`;

      // Set the "create hidden bookmark" URL appropriately
      document.querySelector("#hidden").href =
        `https://purifyplay.github.io/redirection/site/hidden/#${hash}`;
      return;
    }
