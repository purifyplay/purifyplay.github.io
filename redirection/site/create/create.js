...
async function onEncrypt() {
  ...
  const encrypted = await generateFragment(url, password, hint, useRandomSalt,
      useRandomIv);
  const output = `https://purifyplay.github.io/redirection/site/#${encrypted}`;

  document.querySelector("#output").value = output;
  highlight("output");

  // Adjust "Hidden Bookmark" link
  document.querySelector("#bookmark").href = `https://purifyplay.github.io/redirection/site/hidden/#${encrypted}`;
...
function onIvCheck(checkbox) {
  if (!checkbox.checked) {
    checkbox.checked = !confirm("Solo deshabilite el vector de inicialización "
        + "aleatorización si sabes lo que estás haciendo. Deshabilitar esto es "
        + "perjudicial para la seguridad de su enlace encriptado, y solo "
        + "guarda 20-25 caracteres en la longitud de la URL.\n\nPresione \"Cancelar\" a menos que "
        + "estás muy seguro de que sabes lo que estás haciendo.");
  }
